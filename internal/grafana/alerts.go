...
func DefaultThresholds() AlertThresholds {
	return AlertThresholds{
		...
		DiskWarning:    80.0,
		DiskCritical:   90.0,
		NetworkWarning: 1000000000, // 1GB/s
		NetworkCritical: 5000000000, // 5GB/s
	}
}

// NewAlertBundle creates a new alert bundle for a namespace
...
// AddFromMetricValues creates alert rules from metric exports
func (ab *AlertBundle) AddFromMetricValues(ns string, metrics []collector.MetricExport) {
	...
	// Group metrics by type for organized alert rules
	...
	for metricType, metricList := range grouped {
		...
		for _, metric := range metricList {
			// Create warning and critical alerts for each metric
...
// AddFromMetricInfo creates alert rules from CES metric info
...
// groupMetricsByType groups metrics by their logical type (cpu, memory, disk, network, etc.)
...
// groupCESMetricsByType groups CES metrics by their logical type
...
// determineMetricType determines the logical type of a metric based on its name
func (ab *AlertBundle) determineMetricType(metricName string) string {
	...
	case strings.Contains(lowerName, "disk") || strings.Contains(lowerName, "storage"):
		return "disk"
	case strings.Contains(lowerName, "network") || strings.Contains(lowerName, "net") || 
		 strings.Contains(lowerName, "bandwidth") || strings.Contains(lowerName, "bytes"):
		return "network"
...
// createAlertRule creates an alert rule for a metric export
func (ab *AlertBundle) createAlertRule(ns, service string, metric collector.MetricExport, severity string, thresholds AlertThresholds) *AlertRule {
	...
	rule := &AlertRule{
		...
		Annotations: map[string]string{
			"description": fmt.Sprintf("%s %s is %s %.2f%s for {{ $labels.resource_name }}", 
				formatTitle(metric.MetricName), strings.Title(service), ab.getOperatorText(operator), threshold, metric.Unit),
...
// createCESAlertRule creates an alert rule for a CES metric
...
func (ab *AlertBundle) getThreshold(metricType, severity string, thresholds AlertThresholds) float64 {
	...
	default:
		return 0 // No threshold for unknown types
	}
}
